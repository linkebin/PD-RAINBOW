...
function initials() {//公众号排序
    ...
    function asc_sort(a, b) {
        return makePy($(b).find('.num_name').attr('title').charAt(0))[0].toUpperCase() < makePy($(a).find('.num_name').attr('title').charAt(0))[0].toUpperCase() ? 1 : -1;
    }

    var initials = [];
    var num=0;
    SortList.each(function(i) {
        var initial = makePy($(this).find('.num_name').attr('title').charAt(0))[0].toUpperCase();
    ...
    for (var i =0;i<SortList.length;i++) {//插入到对应的首字母后面
        var letter=makePy(SortList.eq(i).find('.num_name').attr('title').charAt(0))[0].toUpperCase();
        switch(letter){
            case "A":
